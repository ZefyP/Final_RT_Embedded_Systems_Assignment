...
/* Standard format to declare the main thread function that is defined later in the code*/
...
void Blink_LED_Thread (void const *argument) {
	...
	uint8_t LED_off = 0; // Defines parameter for LED off
		...
		while (1) { // Creates an infinite loop so that the blinking never terminates
			...
			osDelay(1000); 			/* Uses the built in delay function for the OS to create a 1 second delay. The fundamental delay is specified 
